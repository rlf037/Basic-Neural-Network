...
class NeuralNetwork:
	...
	def input(self, data=None, target=None, flatten=False, problem=None):
		...
		if problem not in ['classification', 'regression']:
			raise ValueError(f"Missing problem='classification' or problem='regression'")

		if data.shape[0] != target.shape[0]:
			raise ValueError(f"X and Y sample sizes are not equal ({data.shape[0]} vs. {target.shape[0]})")
	...
	def transform(self, transform):
		...
		if transform not in trans:
			raise ValueError(f"'{transform}' is not a valid transform. Options: {trans}")
	...
	def split(self, test_split=1/7, shuffle=True, seed=None):
		...
		if self.verbose:
			print(f"Split:\t\t{int(self.X_train.shape[0]):,} ({1.0-test_split:.2%}) Train - {int(self.X_test.shape[0]):,} ({test_split:.2%}) Test")
	...
	def output(self, activation='softmax'):
		...
		self.output_weights = 0.10 * np.random.randn(self.previous_layer_size, self.output_size)
	...
	def compile(self, valid_split=0.2, optimizer='adam',  scorer='accuracy', early_stoppage=True):
	...
	def train(self, batch_size=32, epochs=10):
		...
		# multiple hidden layers	
			...
			for layer_num in range(len(self.weights)):
				...
				self.last_output = np.dot(self.X_train, self.weights[layer_num]) + self.biases[layer_num]
	...
	def splitCheck(self):
		if not self.splitted:
			raise NotImplementedError(f'You have not split the data into training and test sets yet. Use split()')
